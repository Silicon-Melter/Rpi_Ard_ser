...
class SendArduino(Node):
    def __init__(self):
        ...
        self.declare_parameter('echo_mavros', False)  # True: print MAVROS; False: silent MAVROS

        self.port         = self.get_parameter('port').value
        self.baud         = int(self.get_parameter('baud').value)
        self.rate_hz      = float(self.get_parameter('rate_hz').value)
        self.echo_mavros  = bool(self.get_parameter('echo_mavros').value)

        # ===== Serial (always try to open; we always echo Arduino RX) =====
        self.ser = None
        try:
        ...
        self.yaw_deg = 0.0      # deg ENU
        self.roll_deg = 0.0     # deg
        self.lpwm = 1500        # µs (optional, for full-auto PWM pipe-through)
        self.rpwm = 1500        # µs
        ...
        self.get_logger().info(
            f"Echo MAVROS: {self.echo_mavros}. Arduino serial is always echoed."
        )
    ...
    # ===== TX: Pi → Arduino (always sends if serial is up) =====
    def tick_send(self):
